                        ...
                        if (value == 0l) {
                                switch (t) {
                                        case 1:
                                                ...
                                                break;
                                        case 18:
                                                value = (int32)BsonIterInt64(bsonIterator);
                                                break;
                                }
                        }
		...
		switch (t)
		{
			...
			case BSON_TYPE_BOOL:
				appendStringInfoString(
						output, BsonIterBool(&i) ? "true" : "false");
				break;
			...
			default:
				ereport(ERROR, (errcode(ERRCODE_FDW_INVALID_DATA_TYPE),
								errmsg("unsupported BSON type: %x", t)));
		}
 ...
 */
static int
MongoAcquireSampleRows1(Relation relation, int errorLevel,
                                           HeapTuple *sampleRows, int targetRowCount,
                                           double *totalRowCount, double *totalDeadRowCount)
{
        (*totalRowCount) = 100000000;//rowCount;
        (*totalDeadRowCount) = 0;

        return 0;//sampleRowCount;
}
static int
